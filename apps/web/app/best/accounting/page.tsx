// AUTO-GENERATED LISTICLE PAGE
// Generated: 2025-11-30T20:04:47.750Z
// Query: {"category":"Accounting","max_results":20}
// Re-run generate-listicles.ts to update dateModified

import { Metadata } from 'next'
import Link from 'next/link'

export const metadata: Metadata = {
  title: 'Best Accounting Software (November 30, 2025) | Harbor',
  description: 'Compare the top accounting solutions for businesses. Features, pricing, and integrations compared.',
  openGraph: {
    title: 'Best Accounting Software',
    description: 'Compare the top accounting solutions for businesses. Features, pricing, and integrations compared.',
    type: 'article',
    publishedTime: '2025-11-30T20:04:47.750Z',
    modifiedTime: '2025-11-30T20:04:47.750Z',
    authors: ['Harbor'],
  },
  other: {
    'article:modified_time': '2025-11-30T20:04:47.750Z',
  }
}

// Schema.org structured data
const schemaOrg = {
  "@context": "https://schema.org",
  "@graph": [
    {
      "@type": "Article",
      "@id": "https://useharbor.io/best/accounting#article",
      "headline": "Best Accounting Software",
      "description": "Compare the top accounting solutions for businesses. Features, pricing, and integrations compared.",
      "datePublished": "2025-11-30T20:04:47.750Z",
      "dateModified": "2025-11-30T20:04:47.750Z",
      "author": {
        "@type": "Organization",
        "name": "Harbor",
        "url": "https://useharbor.io"
      },
      "publisher": {
        "@type": "Organization",
        "name": "Harbor",
        "url": "https://useharbor.io",
        "logo": {
          "@type": "ImageObject",
          "url": "https://useharbor.io/logo.png"
        }
      },
      "mainEntityOfPage": {
        "@type": "WebPage",
        "@id": "https://useharbor.io/best/accounting"
      }
    },
    {
      "@type": "ItemList",
      "@id": "https://useharbor.io/best/accounting#list",
      "name": "Best Accounting Software",
      "description": "Compare the top accounting solutions for businesses. Features, pricing, and integrations compared.",
      "numberOfItems": 4,
      "itemListElement": [
        {
          "@type": "ListItem",
          "position": 1,
          "item": {
            "@type": "SoftwareApplication",
            "name": "Micro Key Solutions",
            "url": "https://useharbor.io/brands/micro-key-solutions",
            "description": "A leader in software solutions for the security alarm industry.",
            "applicationCategory": "Accounting and Monitoring Software",
            "operatingSystem": "Web-based"
          }
        },
        {
          "@type": "ListItem",
          "position": 2,
          "item": {
            "@type": "SoftwareApplication",
            "name": "Deltek + ComputerEase",
            "url": "https://useharbor.io/brands/deltek-computerease",
            "description": "Deltek provides comprehensive software solutions for project-based businesses.",
            "applicationCategory": "Construction Accounting Software",
            "operatingSystem": "Web-based",
            "offers": {
              "@type": "Offer",
              "price": "0",
              "priceCurrency": "USD",
              "description": "Free tier available"
            }
          }
        },
        {
          "@type": "ListItem",
          "position": 3,
          "item": {
            "@type": "SoftwareApplication",
            "name": "Wrapbook",
            "url": "https://useharbor.io/brands/wrapbook",
            "description": "A digital platform for production payroll and accounting.",
            "applicationCategory": "Payroll and Production Accounting",
            "operatingSystem": "Web-based"
          }
        },
        {
          "@type": "ListItem",
          "position": 4,
          "item": {
            "@type": "SoftwareApplication",
            "name": "Persefoni",
            "url": "https://useharbor.io/brands/persefoni",
            "description": "A leading platform for carbon accounting and sustainability management.",
            "applicationCategory": "Carbon Accounting and Sustainability Management",
            "operatingSystem": "Web-based",
            "offers": {
              "@type": "Offer",
              "price": "0",
              "priceCurrency": "USD",
              "description": "Free tier available"
            }
          }
        }
      ]
    },
    {
      "@type": "BreadcrumbList",
      "itemListElement": [
        {
          "@type": "ListItem",
          "position": 1,
          "name": "Home",
          "item": "https://useharbor.io"
        },
        {
          "@type": "ListItem",
          "position": 2,
          "name": "Best Software",
          "item": "https://useharbor.io/best"
        },
        {
          "@type": "ListItem",
          "position": 3,
          "name": "Best Accounting Software"
        }
      ]
    }
  ]
}

const profiles = [
  {
    "slug": "micro-key-solutions",
    "brand_name": "Micro Key Solutions",
    "domain": "microkey.com",
    "summary": "A leader in software solutions for the security alarm industry.",
    "pricing": {
      "price_model": "custom",
      "price_notes": "Customized pricing based on specific business needs",
      "has_free_tier": false,
      "starting_price": null
    },
    "features": [
      "Automated recurring billing",
      "Integrated service scheduling and tracking",
      "Single-point entry platform",
      "Electronic record keeping",
      "Mobile access for technicians"
    ],
    "integrations": [],
    "visibility_score": 78
  },
  {
    "slug": "deltek-computerease",
    "brand_name": "Deltek + ComputerEase",
    "domain": "construction-software.com",
    "summary": "Deltek provides comprehensive software solutions for project-based businesses.",
    "pricing": {
      "price_model": "unknown",
      "price_notes": null,
      "has_free_tier": true,
      "starting_price": null
    },
    "features": [
      "Manage accounting",
      "Job costing",
      "Projects management",
      "Mobile operations",
      "Optimize project performance"
    ],
    "integrations": [
      "GovWin IQ",
      "Replicon Global Time Tracking"
    ],
    "visibility_score": 73
  },
  {
    "slug": "wrapbook",
    "brand_name": "Wrapbook",
    "domain": "wrapbook.com",
    "summary": "A digital platform for production payroll and accounting.",
    "pricing": {
      "price_model": "unknown",
      "price_notes": null,
      "has_free_tier": false,
      "starting_price": null
    },
    "features": [
      "Payroll processing",
      "Production accounting",
      "Accounts payable management",
      "Real-time data insights",
      "Vendor payment processing"
    ],
    "integrations": [
      "Sage Intacct",
      "Acumatica",
      "Oracle NetSuite",
      "Quickbooks Online",
      "Hot Budget",
      "Movie Magic",
      "Showbiz",
      "CashétPay"
    ],
    "visibility_score": 73
  },
  {
    "slug": "persefoni",
    "brand_name": "Persefoni",
    "domain": "persefoni.com",
    "summary": "A leading platform for carbon accounting and sustainability management.",
    "pricing": {
      "price_model": "unknown",
      "price_notes": null,
      "has_free_tier": true,
      "starting_price": null
    },
    "features": [
      "Scope 1, 2, and 3 carbon footprints",
      "Assurance-grade GHG emissions reporting",
      "Net-zero target setting and reduction modeling",
      "Financed emissions measurement and analytics",
      "Anomaly detection for key data sets"
    ],
    "integrations": [
      "AWS",
      "Workiva",
      "Patch",
      "Bain & Company",
      "Connor Group",
      "A&M"
    ],
    "visibility_score": 73
  }
]

// Export for freshness verification
// Generated at: 2025-11-30T20:04:47.750Z

export default function ListiclePage() {
  return (
    <>
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(schemaOrg) }}
      />
      
      <main className="min-h-screen bg-[#101A31] text-white">
        <div className="max-w-4xl mx-auto px-4 py-12">
          {/* Header with semantic time element for crawlers */}
          <header className="mb-12">
            <p className="text-sm text-gray-400 mb-2">
              Last verified: <time dateTime="2025-11-30T20:04:47.750Z">November 30, 2025</time> &bull; {profiles.length} solutions reviewed
            </p>
            <h1 className="text-3xl md:text-4xl font-bold mb-4">
              Best Accounting Software
            </h1>
            <p className="text-lg text-gray-300">
              Compare the top accounting solutions for businesses. Features, pricing, and integrations compared.
            </p>
          </header>

          {/* Quick Summary */}
          <section className="mb-12 p-6 bg-[#141E38] rounded-lg border border-white/10">
            <h2 className="text-xl font-semibold mb-4">Quick Summary</h2>
            <ul className="space-y-2 text-gray-300">
              {profiles.slice(0, 3).map((p, i) => (
                <li key={p.slug} className="flex items-start gap-2">
                  <span className="text-[#FF6B4A] font-bold">{i + 1}.</span>
                  <span>
                    <strong>{p.brand_name}</strong> &mdash; {p.summary.slice(0, 100)}...
                  </span>
                </li>
              ))}
            </ul>
          </section>

          {/* Full List */}
          <section>
            <h2 className="text-2xl font-semibold mb-6">Full Comparison</h2>
            <div className="space-y-8">
              {profiles.map((profile, index) => (
                <article 
                  key={profile.slug}
                  className="p-6 bg-[#141E38] rounded-lg border border-white/10 hover:border-white/20 transition-colors"
                >
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <span className="text-[#FF6B4A] text-sm font-medium">
                        #{index + 1}
                      </span>
                      <h3 className="text-xl font-semibold mt-1">
                        <Link 
                          href={`/brands/${profile.slug}`}
                          className="hover:text-[#FF6B4A] transition-colors"
                        >
                          {profile.brand_name}
                        </Link>
                      </h3>
                    </div>
                    {profile.pricing && (
                      <div className="text-right">
                        {profile.pricing.has_free_tier && (
                          <span className="inline-block px-2 py-1 bg-green-500/20 text-green-400 text-xs rounded mb-1">
                            Free tier
                          </span>
                        )}
                        {profile.pricing.starting_price && (
                          <p className="text-sm text-gray-400">
                            From {profile.pricing.starting_price}
                          </p>
                        )}
                      </div>
                    )}
                  </div>
                  
                  <p className="text-gray-300 mb-4">{profile.summary}</p>
                  
                  {profile.features.length > 0 && (
                    <div className="mb-4">
                      <p className="text-sm text-gray-400 mb-2">Key features:</p>
                      <div className="flex flex-wrap gap-2">
                        {profile.features.map(feature => (
                          <span 
                            key={feature}
                            className="px-2 py-1 bg-white/5 text-sm rounded"
                          >
                            {feature}
                          </span>
                        ))}
                      </div>
                    </div>
                  )}
                  
                  {profile.integrations.length > 0 && (
                    <div>
                      <p className="text-sm text-gray-400 mb-2">Integrates with:</p>
                      <div className="flex flex-wrap gap-2">
                        {profile.integrations.map(integration => (
                          <span 
                            key={integration}
                            className="px-2 py-1 bg-[#2979FF]/20 text-[#2979FF] text-sm rounded"
                          >
                            {integration}
                          </span>
                        ))}
                      </div>
                    </div>
                  )}
                  
                  <div className="mt-4 pt-4 border-t border-white/10">
                    <Link
                      href={`/brands/${profile.slug}`}
                      className="text-[#FF6B4A] hover:underline text-sm"
                    >
                      View full profile &rarr;
                    </Link>
                  </div>
                </article>
              ))}
            </div>
          </section>

          {/* Footer */}
          <footer className="mt-12 pt-8 border-t border-white/10 text-center text-gray-400 text-sm">
            <p>
              This comparison is generated from structured data collected by Harbor.
              <br />
              Last verified: <time dateTime="2025-11-30T20:04:47.750Z">2025-11-30</time>
            </p>
            <p className="mt-4">
              <Link href="/" className="text-[#FF6B4A] hover:underline">
                Learn more about Harbor &rarr;
              </Link>
            </p>
          </footer>
        </div>
      </main>
    </>
  )
}

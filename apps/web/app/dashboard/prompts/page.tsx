...
import { 
  MessageSquare, Plus, Search, Upload, ChevronDown, ChevronRight,
  MoreHorizontal, X, Sparkles, Eye, Smile, ArrowUpDown, AtSign, BarChart3, Tag
} from 'lucide-react'
...
export default function PromptsPage() {
  ...
  const filtered = current.filter(p => !search || p.prompt_text.toLowerCase().includes(search.toLowerCase()))
  ...
  return (
    <div className="min-h-screen bg-page pb-12">
      ...
      <div className="px-6 flex items-center gap-1 mb-4">
        {(['active', 'suggested', 'inactive'] as TabId[]).map(t => (
          ...
          >
            ...
            {t === 'active' && ` (${prompts.length})`}
            {t === 'suggested' && ` (${suggested.length})`}
      ...
      <div className="mx-6 card p-0 overflow-hidden">
        ...
        <div className="flex items-center justify-end gap-3 px-4 py-3 border-b border-border">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search"
              className="input pl-9 h-9 w-48 text-sm"
            />
          </div>
          <button className="btn-secondary h-9 text-sm">
        ...
        {/* Table Header */}
        {tab === 'suggested' ? (
          <div className="grid grid-cols-[1fr,100px,100px,140px] gap-4 px-4 py-2 bg-secondary/50 border-b border-border text-xs font-medium text-muted uppercase tracking-wide">
            <div className="flex items-center gap-1">Suggested Prompt <ChevronDown className="w-3 h-3" /></div>
            <div className="flex items-center gap-1"><BarChart3 className="w-3 h-3" /> Volume <span className="text-[10px] text-accent bg-accent/10 px-1 rounded normal-case font-normal">Beta</span></div>
            <div>Suggested At</div>
            <div></div>
          </div>
        ) : (
          <div className="grid grid-cols-[1fr,70px,70px,70px,70px,90px,60px] gap-3 px-4 py-2 bg-secondary/50 border-b border-border text-xs font-medium text-muted uppercase tracking-wide">
            <div className="flex items-center gap-1">Prompt <ChevronDown className="w-3 h-3" /></div>
            <div className="flex items-center gap-1"><Eye className="w-3 h-3" /> Visibility</div>
            <div className="flex items-center gap-1"><Smile className="w-3 h-3" /> Sentiment</div>
            <div className="flex items-center gap-1"><ArrowUpDown className="w-3 h-3" /> Position</div>
            <div className="flex items-center gap-1"><AtSign className="w-3 h-3" /> Mentions</div>
            <div className="flex items-center gap-1"><BarChart3 className="w-3 h-3" /> Volume <span className="text-[10px] text-accent bg-accent/10 px-1 rounded normal-case font-normal">Beta</span></div>
            <div className="flex items-center gap-1"><Tag className="w-3 h-3" /> Tags</div>
          </div>
        )}
              ...
              <div key={topic} className="border-b border-border last:border-b-0">
                ...
                <div
                  ...
                  className={`group cursor-pointer hover:bg-hover transition-colors ${
                    tab === 'suggested' 
                      ? 'grid grid-cols-[1fr,100px,100px,140px] gap-4' 
                      : 'grid grid-cols-[1fr,70px,70px,70px,70px,90px,60px] gap-3'
                  } px-4 py-3`}
                >
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={e => e.stopPropagation()} className="ml-auto opacity-0 group-hover:opacity-100 p-1 hover:bg-secondary rounded">
                      <MoreHorizontal className="w-4 h-4 text-muted" />
                    </button>
                  </div>
                  {tab !== 'suggested' && (
                    <>
                      <div className="text-sm text-muted">0%</div>
                ...
                {expanded.has(topic) && items.map(p => (
                  <div
                    key={p.id}
                    onClick={() => tab !== 'suggested' && router.push(`/dashboard/prompts/${p.id}`)}
                    className={`border-t border-border hover:bg-hover transition-colors ${
                      tab !== 'suggested' ? 'cursor-pointer' : ''
                    } ${
                      tab === 'suggested'
                        ? 'grid grid-cols-[1fr,100px,100px,140px] gap-4'
                        : 'grid grid-cols-[1fr,70px,70px,70px,70px,90px,60px] gap-3'
                  ...
                  >
                    <div className="flex items-center gap-3 pl-6">
                      ...
                      <span className="text-sm text-secondary">{p.prompt_text}</span>
                    </div>

...
// Split-screen signup with visual right panel
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-page">
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            ...
            <p className="text-secondary">Start tracking your AI visibility in minutes</p>
          </div>
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <div>
              ...
              <div className="mt-2 space-y-1">
                <div className={`flex items-center gap-2 text-xs ${hasMinLength ? 'text-emerald-500' : 'text-muted'}`}>
                  <Check className={`w-3 h-3 ${hasMinLength ? 'opacity-100' : 'opacity-30'}`} />
                  At least 8 characters
                </div>
                <div className={`flex items-center gap-2 text-xs ${hasUppercase ? 'text-emerald-500' : 'text-muted'}`}>
                  <Check className={`w-3 h-3 ${hasUppercase ? 'opacity-100' : 'opacity-30'}`} />
                  One uppercase letter
                </div>
                <div className={`flex items-center gap-2 text-xs ${hasNumber ? 'text-emerald-500' : 'text-muted'}`}>
        ...
        <div className="relative w-full max-w-md">
          ...
          <div className="relative space-y-6">
            <h2 className="text-3xl font-bold text-white mb-8">
              Track your visibility<br />across AI models
            </h2>
            ...
            {[
              {
                ...
                description: 'See how ChatGPT, Claude, Gemini, and Perplexity describe your brand'
              },
              {
                title: 'Competitor intelligence',
                description: 'Know which prompts mention your competitors but not you'
              },
              {
                title: 'Actionable insights',
                description: 'Get specific recommendations to improve your AI visibility'
              }
            ].map((feature, i) => (
              ...
              <div className="flex items-center gap-3">
                <div className="flex -space-x-2">
                  {[1, 2, 3, 4].map((i) => (
                    <div 
                      key={i} 
                      className="w-8 h-8 rounded-full bg-gradient-to-br from-gray-600 to-gray-800 border-2 border-[#0a0a0a]"
                    />
                  ))}
                </div>
                <p className="text-white/60 text-sm">
                  Join 500+ brands tracking their AI visibility
                </p>
              </div>
            </div>
          </div>

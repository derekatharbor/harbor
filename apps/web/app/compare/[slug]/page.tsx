// ISR Dynamic Route for /compare/[slug]
// Slug format: "brand1-vs-brand2"
// Revalidates every 24 hours
import { Metadata } from 'next'
import Link from 'next/link'
import { notFound } from 'next/navigation'
import { createClient } from '@supabase/supabase-js'

// Revalidate every 24 hours
export const revalidate = 86400

// Initialize Supabase client
const supabase = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL!,
  process.env.SUPABASE_SERVICE_ROLE_KEY!
)

// Profile type
interface Profile {
  slug: string
  brand_name: string
  domain: string
  category: string
  visibility_score: number
  feed_data: {
    one_line_summary?: string
    short_description?: string
    pricing?: {
      price_model: string
      price_notes: string | null
      has_free_tier: boolean
      starting_price: string | null
    }
    features?: string[]
    integrations?: string[]
  } | null
}

// Parse comparison slug to get both brand slugs
function parseComparisonSlug(slug: string): { slug1: string; slug2: string } | null {
  const parts = slug.split('-vs-')
  if (parts.length !== 2) return null
  return { slug1: parts[0], slug2: parts[1] }
}

// Fetch both brands
async function fetchBrands(slug1: string, slug2: string): Promise<{
  brand1: Profile | null
  brand2: Profile | null
}> {
  const { data, error } = await supabase
    .from('ai_profiles')
    .select('slug, brand_name, domain, category, visibility_score, feed_data')
    .in('slug', [slug1, slug2])

  if (error || !data || data.length < 2) {
    return { brand1: null, brand2: null }
  }

  const brand1 = data.find(b => b.slug === slug1) as Profile | undefined
  const brand2 = data.find(b => b.slug === slug2) as Profile | undefined

  return { 
    brand1: brand1 || null, 
    brand2: brand2 || null 
  }
}

// Generate metadata
export async function generateMetadata({ 
  params 
}: { 
  params: Promise<{ slug: string }> 
}): Promise<Metadata> {
  const { slug } = await params
  const parsed = parseComparisonSlug(slug)
  
  if (!parsed) {
    return { title: 'Not Found' }
  }

  const { brand1, brand2 } = await fetchBrands(parsed.slug1, parsed.slug2)
  
  if (!brand1 || !brand2) {
    return { title: 'Not Found' }
  }

  const now = new Date()
  const displayDate = now.toLocaleDateString('en-US', { 
    year: 'numeric', 
    month: 'long', 
    day: 'numeric' 
  })

  return {
    title: `${brand1.brand_name} vs ${brand2.brand_name} (${displayDate}) | Harbor`,
    description: `Detailed comparison of ${brand1.brand_name} and ${brand2.brand_name}. Features, pricing, and integrations compared side-by-side.`,
    openGraph: {
      title: `${brand1.brand_name} vs ${brand2.brand_name}`,
      description: `Detailed comparison of ${brand1.brand_name} and ${brand2.brand_name}.`,
      type: 'article',
      modifiedTime: now.toISOString(),
    },
  }
}

// Main page component
export default async function ComparePage({ 
  params 
}: { 
  params: Promise<{ slug: string }> 
}) {
  const { slug } = await params
  const parsed = parseComparisonSlug(slug)
  
  if (!parsed) {
    notFound()
  }

  const { brand1, brand2 } = await fetchBrands(parsed.slug1, parsed.slug2)
  
  if (!brand1 || !brand2) {
    notFound()
  }

  const now = new Date()
  const isoDate = now.toISOString()
  const displayDate = now.toLocaleDateString('en-US', { 
    year: 'numeric', 
    month: 'long', 
    day: 'numeric' 
  })

  // Find shared integrations
  const brand1Integrations = new Set(brand1.feed_data?.integrations?.map(i => i.toLowerCase()) || [])
  const sharedIntegrations = brand2.feed_data?.integrations?.filter(i => 
    brand1Integrations.has(i.toLowerCase())
  ) || []

  // Schema.org structured data
  const schemaOrg = {
    '@context': 'https://schema.org',
    '@type': 'Article',
    '@id': `https://useharbor.io/compare/${slug}#article`,
    'headline': `${brand1.brand_name} vs ${brand2.brand_name}: Comparison`,
    'description': `Detailed comparison of ${brand1.brand_name} and ${brand2.brand_name}. Features, pricing, and integrations compared side-by-side.`,
    'datePublished': isoDate,
    'dateModified': isoDate,
    'author': {
      '@type': 'Organization',
      'name': 'Harbor',
      'url': 'https://useharbor.io'
    },
    'about': [
      {
        '@type': 'SoftwareApplication',
        'name': brand1.brand_name,
        'url': `https://useharbor.io/brands/${brand1.slug}`
      },
      {
        '@type': 'SoftwareApplication',
        'name': brand2.brand_name,
        'url': `https://useharbor.io/brands/${brand2.slug}`
      }
    ]
  }

  return (
    <>
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(schemaOrg) }}
      />
      
      <main className="min-h-screen bg-[#101A31] text-white">
        <div className="max-w-5xl mx-auto px-4 py-12">
          {/* Header */}
          <header className="mb-12 text-center">
            <p className="text-sm text-gray-400 mb-2">
              Last verified: <time dateTime={isoDate}>{displayDate}</time>
            </p>
            <h1 className="text-3xl md:text-4xl font-bold mb-4">
              {brand1.brand_name} vs {brand2.brand_name}
            </h1>
            <p className="text-lg text-gray-300 max-w-2xl mx-auto">
              A detailed comparison to help you choose between {brand1.brand_name} and {brand2.brand_name} 
              for your {brand1.category || brand2.category || 'software'} needs.
            </p>
          </header>

          {/* Side by side comparison */}
          <section className="grid md:grid-cols-2 gap-6 mb-12">
            {/* Brand 1 */}
            <div className="p-6 bg-[#141E38] rounded-lg border border-white/10">
              <div className="text-center mb-6 pb-6 border-b border-white/10">
                <h2 className="text-2xl font-bold">{brand1.brand_name}</h2>
                <p className="text-gray-400 text-sm">{brand1.domain}</p>
                {brand1.feed_data?.pricing?.has_free_tier && (
                  <span className="inline-block mt-2 px-2 py-1 bg-green-500/20 text-green-400 text-xs rounded">
                    Free tier available
                  </span>
                )}
              </div>
              
              <p className="text-gray-300 mb-6">
                {brand1.feed_data?.one_line_summary || brand1.feed_data?.short_description}
              </p>
              
              {brand1.feed_data?.pricing?.starting_price && (
                <div className="mb-4">
                  <p className="text-sm text-gray-400">Starting price</p>
                  <p className="text-xl font-semibold text-[#FF6B4A]">{brand1.feed_data.pricing.starting_price}</p>
                </div>
              )}
              
              <div className="mb-4">
                <p className="text-sm text-gray-400 mb-2">Key features ({brand1.feed_data?.features?.length || 0})</p>
                <ul className="space-y-1">
                  {brand1.feed_data?.features?.slice(0, 8).map(feature => (
                    <li key={feature} className="text-sm flex items-center gap-2">
                      <span className="text-green-400">✓</span> {feature}
                    </li>
                  ))}
                </ul>
              </div>
              
              <div className="mb-4">
                <p className="text-sm text-gray-400 mb-2">Integrations ({brand1.feed_data?.integrations?.length || 0})</p>
                <div className="flex flex-wrap gap-1">
                  {brand1.feed_data?.integrations?.slice(0, 10).map(integration => (
                    <span key={integration} className="px-2 py-1 bg-[#2979FF]/20 text-[#2979FF] text-xs rounded">
                      {integration}
                    </span>
                  ))}
                </div>
              </div>
              
              <Link
                href={`/brands/${brand1.slug}`}
                className="block text-center mt-6 py-2 px-4 bg-[#FF6B4A] text-white rounded hover:bg-[#FF6B4A]/90 transition-colors"
              >
                View {brand1.brand_name} Profile
              </Link>
            </div>

            {/* Brand 2 */}
            <div className="p-6 bg-[#141E38] rounded-lg border border-white/10">
              <div className="text-center mb-6 pb-6 border-b border-white/10">
                <h2 className="text-2xl font-bold">{brand2.brand_name}</h2>
                <p className="text-gray-400 text-sm">{brand2.domain}</p>
                {brand2.feed_data?.pricing?.has_free_tier && (
                  <span className="inline-block mt-2 px-2 py-1 bg-green-500/20 text-green-400 text-xs rounded">
                    Free tier available
                  </span>
                )}
              </div>
              
              <p className="text-gray-300 mb-6">
                {brand2.feed_data?.one_line_summary || brand2.feed_data?.short_description}
              </p>
              
              {brand2.feed_data?.pricing?.starting_price && (
                <div className="mb-4">
                  <p className="text-sm text-gray-400">Starting price</p>
                  <p className="text-xl font-semibold text-[#FF6B4A]">{brand2.feed_data.pricing.starting_price}</p>
                </div>
              )}
              
              <div className="mb-4">
                <p className="text-sm text-gray-400 mb-2">Key features ({brand2.feed_data?.features?.length || 0})</p>
                <ul className="space-y-1">
                  {brand2.feed_data?.features?.slice(0, 8).map(feature => (
                    <li key={feature} className="text-sm flex items-center gap-2">
                      <span className="text-green-400">✓</span> {feature}
                    </li>
                  ))}
                </ul>
              </div>
              
              <div className="mb-4">
                <p className="text-sm text-gray-400 mb-2">Integrations ({brand2.feed_data?.integrations?.length || 0})</p>
                <div className="flex flex-wrap gap-1">
                  {brand2.feed_data?.integrations?.slice(0, 10).map(integration => (
                    <span key={integration} className="px-2 py-1 bg-[#2979FF]/20 text-[#2979FF] text-xs rounded">
                      {integration}
                    </span>
                  ))}
                </div>
              </div>
              
              <Link
                href={`/brands/${brand2.slug}`}
                className="block text-center mt-6 py-2 px-4 bg-[#FF6B4A] text-white rounded hover:bg-[#FF6B4A]/90 transition-colors"
              >
                View {brand2.brand_name} Profile
              </Link>
            </div>
          </section>

          {/* Shared integrations */}
          {sharedIntegrations.length > 0 && (
            <section className="mb-12 p-6 bg-[#141E38] rounded-lg border border-white/10">
              <h2 className="text-xl font-semibold mb-4">Shared Integrations</h2>
              <p className="text-gray-400 text-sm mb-4">
                Both {brand1.brand_name} and {brand2.brand_name} integrate with these platforms:
              </p>
              <div className="flex flex-wrap gap-2">
                {sharedIntegrations.map(integration => (
                  <span key={integration} className="px-3 py-1 bg-green-500/20 text-green-400 text-sm rounded">
                    {integration}
                  </span>
                ))}
              </div>
            </section>
          )}

          {/* Related links */}
          <section className="grid md:grid-cols-2 gap-4">
            <Link
              href={`/alternatives/${brand1.slug}`}
              className="p-4 bg-[#141E38] rounded-lg border border-white/10 hover:border-white/20 transition-colors"
            >
              <p className="font-medium">{brand1.brand_name} Alternatives &rarr;</p>
              <p className="text-sm text-gray-400">See more options like {brand1.brand_name}</p>
            </Link>
            <Link
              href={`/alternatives/${brand2.slug}`}
              className="p-4 bg-[#141E38] rounded-lg border border-white/10 hover:border-white/20 transition-colors"
            >
              <p className="font-medium">{brand2.brand_name} Alternatives &rarr;</p>
              <p className="text-sm text-gray-400">See more options like {brand2.brand_name}</p>
            </Link>
          </section>

          {/* Footer */}
          <footer className="mt-12 pt-8 border-t border-white/10 text-center text-gray-400 text-sm">
            <p>
              This comparison is generated from structured data collected by Harbor.
              <br />
              Last verified: <time dateTime={isoDate}>{displayDate}</time>
            </p>
          </footer>
        </div>
      </main>
    </>
  )
}

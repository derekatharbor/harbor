...
export async function GET(
  ...
  try {
    ...
    if (!forceRefresh) {
      ...
      if (cached) {
        ...
        if (hoursSinceCrawl < CACHE_TTL_HOURS) {
          ...
          // Check if cached data is in old RAW format (has pages_analyzed instead of pages_scanned)
          if (results.pages_analyzed !== undefined && results.pages_scanned === undefined) {
            // Old format - re-format it before returning
            console.log('[Website Analytics] Converting old cache format to new format')
            const formatted = formatCrawlResponse(results as CrawlResult)
          ...
          // New format - return as-is with has_data fallback
          return NextResponse.json({
            ...results,
            has_data: results.has_data ?? (results.pages_scanned > 0),
            cached: true,
            crawled_at: cached.crawled_at,
            next_refresh_available: new Date(crawledAt.getTime() + CACHE_TTL_HOURS * 60 * 60 * 1000)
          })
    ...
    // Store FORMATTED results (not raw) so cache returns consistent format
    const { error: insertError } = await supabase
      .from('website_crawls')
      .insert({
        dashboard_id: dashboardId,
        domain: dashboard.domain,
        results: response,  // Store formatted, not raw!

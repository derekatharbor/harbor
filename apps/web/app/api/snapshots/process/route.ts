...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (finalStatus === 'done' || finalStatus === 'partial') {
      console.log('[Process] 📸 Creating scan snapshot...')
      try {
        const snapshotResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/snapshots/create`, {
          method: 'POST',
        ...
        if (snapshotResponse.ok) {
          console.log('[Process] ✅ Snapshot created successfully')
        } else {
          console.error('[Process] ⚠️ Snapshot creation failed:', snapshotResponse.status)
        }
      } catch (snapError) {
        console.error('[Process] ⚠️ Snapshot creation error:', snapError)
        // Don't fail the whole scan if snapshot fails
      }
    }

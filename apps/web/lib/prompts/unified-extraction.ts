...
async function refreshCache(supabase: SupabaseClient) {
  ...
  if (brands) {
    ...
    for (const brand of brands) {
      ...
      brandCache.set(brand.name.toLowerCase(), { id: brand.id, name: brand.name })
      if (brand.slug) {
        brandCache.set(brand.slug.toLowerCase(), { id: brand.id, name: brand.name })
      }
      // Handle common variations
      const simplified = brand.name.toLowerCase().replace(/[^a-z0-9]/g, '')
      brandCache.set(simplified, { id: brand.id, name: brand.name })
    }
  }
...
export async function storeExtractionResults(
  ...
  if (results.brands.length > 0) {
    ...
    await supabase
      ...
      .upsert(brandMentions, { onConflict: 'execution_id,brand_name' })
  }
